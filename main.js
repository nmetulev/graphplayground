...
var userAgentApplication = new Msal.UserAgentApplication(applicationConfig.clientID, applicationConfig.authority, null);
...
async function initWindows()
{
    if (window.Windows)
    {
        ...
        let redirectUri = web.WebAuthenticationBroker.getCurrentApplicationCallbackUri().host;
        ...
        if (wtrr.responseStatus == webCore.WebTokenRequestStatus.success)
        {
            ...
            let token = wtrr.responseData[0].token;
...
{
    graphClient = MicrosoftGraph.Client.init( { authProvider: done => done(null, token) } )
    await getMe();
...
async function getMe()
{
    if (graphClient)
    {
        let result = await graphClient.api('/me').select("displayName").get();
        _$login_div.innerHTML = `<div id='displayname'></div>`;

        _$login_div.querySelector('#displayname').innerHTML = result.displayName
    }
...
async function getAllMeetings()
{
    if (graphClient)
    {
        ...
        let response = await graphClient.api('/me/calendarView?startdatetime=' + now.toISOString() + '&enddatetime=' + nextWeek.toISOString()).get();
